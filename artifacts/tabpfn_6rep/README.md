...
`submissions/submission_tabpfn6.csv` (leaderboard log loss **0.40982**, our best) uses TabPFN
run at **6 fold-partitions with a 3-seed full-data refit**, blended
0.45 LightGBM / 0.30 GRU / 0.25 TabPFN.

`tabpfn_model_6rep.py` here is identical to `models/tabpfn_model.py` in the repo root
...
## Reproducing submissions/submission_tabpfn6.csv without retraining
