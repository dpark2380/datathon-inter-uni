...
`submissions/submission_tabpfn3.csv` (leaderboard log loss **0.40995**) was produced with
TabPFN run at **3 fold-partitions and no full-data refit**. `models/tabpfn_model.py`
...
## Reproducing submissions/submission_tabpfn3.csv without retraining
...
pd.DataFrame({"client_id": ids, "prob_default": preds}).to_csv("submissions/submission_tabpfn3.csv", index=False)
```
